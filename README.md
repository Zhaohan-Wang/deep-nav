...
`标题` → `选关` → `认领岗位` → `飞行` → `自然结算` → 下一关。训练关增加操作理解检查；正式任务 01 在结算后测正常状态基线；正式任务 02、03 在目标异常后的行为窗口完整结束后，才进入事件回顾、责任分配和状态评价。最后一关双方提交后进入 `感谢游玩`。
...
- `missions.csv`：每次航段的时长、路径、损伤、航点统计、结果与中止原因
- `target_events.csv`：唯一核心异常、触发位置、修正、驾驶响应和恢复窗口
- `ratings.csv`：与 `event_id` 连接的异常觉察、责任分配和状态评价
- `waypoints.csv`：每次航点请求、冷却、接受结果、驾驶响应和覆盖结局
