...
var _device_slots_to_seats: Dictionary = {0: 0, 1: 1}
...
func _try_start_listening() -> void:
	...
	if bind_error != OK:
		...
		return
	var executable := ProjectSettings.globalize_path(BRIDGE_PATH)
...
func _handle_message(message: Dictionary) -> void:
	...
	if type == "ready":
		...
		if device_slot < 0 or device_slot > 1:
			return
		var slot := int(_device_slots_to_seats.get(device_slot, device_slot))
		...
		var device_slot := int(message.get("slot", -1))
		mouse_motion.emit(int(_device_slots_to_seats.get(device_slot, device_slot)), Vector2(float(message.get("dx", 0.0)), float(message.get("dy", 0.0))))
	elif type == "button":
		var device_slot := int(message.get("slot", -1))
		mouse_button.emit(int(_device_slots_to_seats.get(device_slot, device_slot)), int(message.get("button", 1)), bool(message.get("pressed", false)))
	elif type == "wheel":
		var device_slot := int(message.get("slot", -1))
		mouse_wheel.emit(int(_device_slots_to_seats.get(device_slot, device_slot)), int(message.get("delta", 0)))
	elif type == "key":
		var device_slot := int(message.get("slot", -1))
		var seat := int(_device_slots_to_seats.get(device_slot, device_slot))
...
func swap_mouse_seats() -> void:
	_device_slots_to_seats[0] = 1 - int(_device_slots_to_seats.get(0, 0))
	_device_slots_to_seats[1] = 1 - int(_device_slots_to_seats.get(1, 1))
	var previous_a := str(_devices.get(0, ""))
	var previous_b := str(_devices.get(1, ""))
	var previous_keyboard_a := str(_keyboards.get(0, ""))
	var previous_keyboard_b := str(_keyboards.get(1, ""))
	_devices.clear()
	if not previous_b.is_empty(): _devices[0] = previous_b
	if not previous_a.is_empty(): _devices[1] = previous_a
	_keyboards.clear()
	if not previous_keyboard_b.is_empty(): _keyboards[0] = previous_keyboard_b
	if not previous_keyboard_a.is_empty(): _keyboards[1] = previous_keyboard_a
	var previous_keys_a := (_pressed_keys[0] as Dictionary).duplicate()
	_pressed_keys[0] = (_pressed_keys[1] as Dictionary).duplicate()
	_pressed_keys[1] = previous_keys_a
	device_changed.emit(0, not previous_b.is_empty(), previous_b)
	device_changed.emit(1, not previous_a.is_empty(), previous_a)
	keyboard_device_changed.emit(0, not previous_keyboard_b.is_empty(), previous_keyboard_b)
	keyboard_device_changed.emit(1, not previous_keyboard_a.is_empty(), previous_keyboard_a)
